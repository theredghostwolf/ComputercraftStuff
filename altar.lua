...
function hasNoItemsToCraft ()
  ...
  elseif currentCraftingMode == craftingModes[2] then
    if findItem(chest, slates[currentSlate]["item"]) then
      return false
    else
      r = findCraftRequirement(slates[currentSlate])
      while r do
        slot = findItem(chest, r["item"])
        if slot then
          return false
        else
...
function pullOrbFromAltar ()
  ...
  if o then
    if enableOrbContainer then
      pushItems(altar, orbChest, findItem(altar, o),1)
    else
      pushItems(altar, chest, findItem(altar, o),1)
    end
  end
end
...
function insertOrbInAltar ()
  if enableOrbContainer then
    if findOrb(orbChest) then
      pushItems(orbChest, altar,findItem(orbChest, findOrb(orbChest)),1)
    end
  else
    if findOrb(chest) then
      pushItems(chest, altar,findItem(chest, findOrb(chest)),1)
    end
  end
end
...
function insertItemToAltar (item, amount)
  pushItems(chest, altar,  findItem(chest, item), amount)
end
...
function pullItemFromAltar (item, amount)
  if enableOutput and (currentCraftingMode == craftingModes[1] or item == slates[currentSlate]["result"]) then
    pushItems(altar, outputChest, findItem(altar, item), amount)
  else
    pushItems(altar, chest, findItem(altar, item), amount)
  end
end
